...
class Dataloader:
    ...
    def __init__(self, mode=None):
        ...
        self.train_visual, self.val_visual, self.test_visual = {}, {}, {}

        # Plan is to have only importing pickle file at the very max at ths=is level
        # Infact that can also be avoided
    ...
    def load_visual_data(self, ):
        ...
        # VISUAL_PATH = "./data/pickles/audio_embeddings_feature_selection_{}.pkl".format(self.MODE.lower())
        VISUAL_PATH = "./data/pickles/visual_{}.pkl".format(self.classification_mode.lower())
        self.train_visual_emb, self.val_visual_emb, self.test_visual_emb = pickle.load(open(VISUAL_PATH, "rb"))
    ...
    def load_trimodal_data(self, ):
        TEXT_UNIMODAL = "./data/pickles/text_{}.pkl".format(self.MODE.lower())
        AUDIO_UNIMODAL = "./data/pickles/audio_{}.pkl".format(self.MODE.lower())
        VISUAL_UNIMODAL = "./data/pickles/visual_{}.pkl".format(self.MODE.lower())

        # Load features
        train_text_x, val_text_x, test_text_x = pickle.load(open(TEXT_UNIMODAL, "rb"), encoding='latin1')
        train_audio_x, val_audio_x, test_audio_x = pickle.load(open(AUDIO_UNIMODAL, "rb"), encoding='latin1')
        train_visual_x, val_visual_x, test_visual_x = pickle.load(open(VISUAL_UNIMODAL, "rb"), encoding='latin1')

        def concatenate_fusion(ID, text, audio, visual):
            trimodal = []
            for vid, utts in ID.items():
                trimodal.append(np.concatenate((text[vid], audio[vid], visual[vid]), axis=1))
            return np.array(trimodal)

        self.train_dialogue_features = concatenate_fusion(self.train_dialogue_ids, train_text_x, train_audio_x, train_visual_x)
        self.val_dialogue_features = concatenate_fusion(self.val_dialogue_ids, val_text_x, val_audio_x, val_visual_x)
        self.test_dialogue_features = concatenate_fusion(self.test_dialogue_ids, test_text_x, test_audio_x, test_visual_x)
